...
function parseValue(value, pointStr, specialValues) {
	...
	if (Number.isNaN(parsed)) {
		...
		if (unit !== 'px' && unit !== '%') {
			throw new Error(
				`"${unit}" is not valid unit for the attachment point "${pointStr}"`
			)
		}
...
function parsePoint(pointStr, useSpecialValues = true) {
	const [horiz, vert] = pointStr.split(' ')
	return {
		horiz: parseValue(
			horiz,
			pointStr,
			useSpecialValues ? HORIZ_SPECIAL_VALUES : undefined
		),
		vert: parseValue(
			vert,
			pointStr,
			useSpecialValues ? VERT_SPECIAL_VALUES : undefined
		)
	}
}
