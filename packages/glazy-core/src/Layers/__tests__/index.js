...
import { Stack, Layer } from '../index.js'
...
describe('layers', () => {
	it('renders layer', () => {
		...
		const layers = testRenderer.root.findAllByType(LayerView)
	...
	it('closes layer when isActive is false', () => {
		...
		assert.equal(testRenderer.root.findAllByType(LayerView).length, 2)

		// set open false
		testRenderer.update(<Test isActive={false}/>)
		// layer is not rendered
		assert.equal(testRenderer.root.findAllByType(LayerView).length, 1)
	})
})
